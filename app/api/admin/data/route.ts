...
export async function GET() {
  ...
  const [menu, menu_categories, reviews, partners, settings] = await Promise.all([
    admin.from("menu_items").select("*").order("menu_group", { ascending: true }).order("sort_order", { ascending: true }),
  ...
  if (menu.error || menu_categories.error || reviews.error || partners.error || settings.error) {
    return NextResponse.json(
      {
        error: "Database error",
        details: [menu.error, menu_categories.error, reviews.error, partners.error, settings.error].filter(Boolean),
      },
      { status: 500 }
    );
  }
  ...
  return NextResponse.json({
    menu: menu.data ?? [],
    menu_categories: menu_categories.data ?? [],
